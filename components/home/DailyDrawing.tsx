...
export default function DailyDrawing({ index }: { index: string }) {
  ...
  const tweaks = useMemo(() => {
    const t: Tweaks = {};
    if (dial.peakSharpness !== DIAL_DEFAULTS.peakSharpness)
      t.sharpness = dial.peakSharpness as number;
    if (dial.mountainHeight !== DIAL_DEFAULTS.mountainHeight)
      t.height = dial.mountainHeight as number;
    if (dial.peakPosition !== DIAL_DEFAULTS.peakPosition)
