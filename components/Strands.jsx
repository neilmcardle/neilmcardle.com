...
export default function Strands({
  ...
  useEffect(() => {
    ...
    // Respect the OS "reduce motion" setting: paint a single static frame and
    // never spin up the animation loop for those users.
    const reduceMotion =
    ...
    // Paint once so reduced-motion users still see the strands and there is no
    // blank flash before the loop starts.
    renderFrame(0);

    // Only burn GPU while the hero is actually on screen and the tab is active.
    let inView = true;
    let pageHidden = typeof document !== 'undefined' && document.hidden;
    const evaluate = () => {
      if (inView && !pageHidden) start();
      else stop();
    };
