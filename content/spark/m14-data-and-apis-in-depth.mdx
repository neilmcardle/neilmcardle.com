...
promise: "Design and build the endpoint that powers your own interface."
module: 15
chapter: 14
...
checkpoint:
  ...
  - "(Optional) endpoint integrated with Data & APIs in depth's database"
difficulty: "intermediate"
prereq: "JavaScript fundamentals (forms), Intermediate JavaScript: Fetch and Real Data (async)"
...
- **Verb:** POST. Tells the server "I want you to _create_ something." (GET = read, POST = create, PUT = update, DELETE = delete)
...
This is everything. A POST with no body is a request. A GET with 20 headers is a request. An image is a request (it returns binary instead of JSON). The _language_ doesn't matter, a request is a request, whether it's coming from a browser, an app, or curl.
...
export async function POST(request: Request) {
  try {
    ...
    // For now, just pretend we saved it. In Databases & ORM you'll use a real database.
...
Next.js provides the _framework_ (route handlers, Response API, server actions), but you need to understand the _language_ (HTTP) first. If you learn Next.js without learning HTTP, you'll hit a wall the moment you need to integrate with a third-party API, debug a network issue, or switch frameworks.
...
This has a huge upside: your endpoint code is portable. The `Response.json()` method works the same way in Next.js, in a Worker, in a Node.js HTTP server. But it also means you need to understand _what_ the platform gives you, not just _how_ Next.js wraps it.
