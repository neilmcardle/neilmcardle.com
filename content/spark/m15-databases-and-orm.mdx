...
threads:
  - language-vs-framework: TypeScript types vs. database reality
  - your-code-vs-the-platform: Your ORM vs. the database engine
---

## Why This Matters

Your data lives here now. Everything you built in Data & APIs in depth ,  the waitlist signups, the blog post drafts, the comment threads ,  was in memory. Refresh the browser, gone. Real products need real persistence.
...
**The thread: language-vs-framework.** Your TypeScript types (`User`, `Post`, `Comment`) are *semantic*. Your database schema is *structural*. A good schema makes your types obvious. A bad schema makes your types confused. Drizzle bridges this gap: your schema *generates* your types, so they stay in sync forever.
...
**The thread: your-code-vs-the-platform.** The migration is your code. The SQL is the platform's language. Drizzle generates the SQL, but you're writing the schema in TypeScript. You stay in your language; the ORM handles the translation. This is the core value of an ORM.
...
Because Drizzle is smart. It fetches all posts in one query, collects all the `user_id`s, then fetches all those users in *one* query. Two queries total, not 101. This is called a "batch fetch."
...
## Build: Persist Data & APIs in depth's Data

Your assignment: take the waitlist and blog platform from Data & APIs in depth and give it real persistence.
...
In your Data & APIs in depth API routes, replace in-memory storage with database calls:
