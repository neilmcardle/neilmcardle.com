...
**Critical gotcha:** `fetch()` does NOT reject on HTTP errors like 404 or 500. It only rejects on network errors (no internet, DNS failed, timeout). A 404 response is still a *successful fetch*. You have to check `response.ok` yourself.
...
export function WeatherCard({ city = 'San Francisco', latitude = 37.7749, longitude = -122.4194 }) {
...
```javascript
const luke = await fetch('https://swapi.dev/api/people/1/')
  .then(r => r.json());
...
**How to debug:**
1. Look at the error message. It tells you *what* broke.
2. Look at the line number. It tells you *where* it broke.
3. The error location is a hint about *why*. Is it a network error? A JSON parse error? A data shape error?
