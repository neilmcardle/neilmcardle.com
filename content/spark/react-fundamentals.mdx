...
<Check
  question='On the fourth render, what does useState("") return?'
  options="An empty string, because that is the argument|Whatever the state currently is, because the argument only applies on the first render|It throws, because state is already set"
  correct="1"
  why="The argument is the initial value, and it is only used once. After the first render React ignores it.|Correct. useState uses its argument on the first render and returns the stored value on every render after that.|Calling useState on every render is exactly how it is designed to work. There is nothing to throw."
/>
...
function BMI() {
  ...
  return (
    <div>
      ...
      <label htmlFor="height">Height in metres</label>
      <input
        id="height"
        value={height}
...
<Check
  question="A search page holds the query in state. Should the filtered results also live in useState?"
  options="Yes, so they persist between renders|No, because they can be computed from the query and the list|Only if the list is large"
  correct="1"
  why="Results do not need to persist. They are rebuilt from the query on every render anyway, and storing them gives you two things that can disagree.|Correct. Anything computable from existing state is derived. Store it and you have to remember to update it, which is how state goes stale.|Size is a performance question, not a correctness one, and it is not the reason to reach for state. Measure before optimising."
/>
...
<Check
  question="Two sibling components need to show the same selected item. Where does that selection belong?"
  options="In each sibling, kept in sync when either changes|In their closest shared parent, passed down as props|In a variable outside both components"
  correct="1"
  why="Two copies of one fact will drift apart the moment you forget to update one of them. Keeping them in sync by hand is the bug, not the fix.|Correct. Lift the state to the nearest parent both siblings share, then pass it down. One source of truth, no syncing.|A variable outside the component is not state. Changing it does not tell React to re-render, so the screen would not update."
/>
...
<Check
  question="A search returns zero results and your UI renders nothing at all. Which state did you skip?"
  options="Loading|Empty|Error"
  correct="1"
  why="Loading is the state while the request is in flight. Here the request finished, it just matched nothing.|Correct. Empty is a successful result with no rows, and it needs its own message. Rendering nothing leaves the user unable to tell success from breakage.|An error means something went wrong. Finding no matches is the system working correctly."
/>
