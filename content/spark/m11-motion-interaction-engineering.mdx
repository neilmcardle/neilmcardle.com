...
promise: "Build motion that communicates, and know when to leave it out."
module: 12
...
Motion is how an interface tells you a story. A button that springs into place when pressed feels alive. A page that fades in feels intentional. A transition that stutters feels broken. The hardest skill in motion design is knowing when _not_ to animate, when silence is louder than movement.
...
Motion isn't just about states, it's about _input_. Framer Motion captures gestures.
...
## 7. Restraint: When _Not_ to Animate
...
The press is _user-triggered_, so immediate visual feedback is worth animating.
...
Each screen transitions via fade + slide. Buttons respond to clicks with spring bounces. Skip buttons exist to jump to the end. The _entire flow_ respects `prefers-reduced-motion`.
