...
promise: "Work the way an engineer on a team works: branches, reviews, and deploys."
...
You will learn to work like a professional engineer: committing thoughtfully, writing PRs that explain _why_, reading code you didn't write, testing components, debugging methodically, deploying safely, and using AI as a thinking partner, not a replacement for thinking.
...
- _When_ it appeared
- _Who_ added it
- _Why_ (in the commit message)
- _What else changed_ with it (the whole commit)
...
When you're done, you _merge_ the branch into main. Git replays your commits on top of main's latest work. This is a PR.
...
- What _type_ of code is this? (Component, utility, hook, API route?)
...
Each step is one line of code (or a few). But _knowing_ the path is the whole skill.

### The Debugger Is Your Eyes

Stop reading and start _watching_.
...
This turns abstract code into _runnable_ code. You can watch it execute.
...
When something breaks, the error message tells you _where_ to look. Most developers ignore it and thrash.
...
A race condition might corrupt data on the server, but the error happens later on the client. Trace the data's _origin_, not just where it died.
...
A Postgres connection string packs the username, password, host and database
name into a single value. That is exactly why `DATABASE_URL` must never be
committed: one leaked line hands over the whole database.
...
- Understanding _why_ you chose this approach
...
- **When your codebase is unique.** AI knows common patterns. It doesn't know _your_ style or conventions. Use it for ideas, not verbatim.
...
4. **Create a PR.** Write a PR description that explains _why_ this matters.
...
Run `npm test` and confirm every unit test is green.
```
...
- [ ] Write a PR description that explains _why_ (not just _what_)
...
- **PRs** are asynchronous communication. Explain _why_ before explaining _what_.
