...
function BookMindContent() {
  ...
  // dropdown) — not here. Redirect them back to the editor. The
  // authLoading + subLoading guard covers both the top-level auth
  // resolution and the subscription fetch. The root race was in
  // useSubscription, which used to collapse "auth in progress" to
  // "anonymous user" and publish isLoading=false with tier='free';
  // that's now fixed there, so this effect is straightforward.
  useEffect(() => {
