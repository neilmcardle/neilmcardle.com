  ...
  return (
    <span
      className={`relative w-4 h-5 shrink-0 flex items-center justify-center ${
        isSelected
          ? "text-white/45 dark:text-[var(--clay-muted)]"
...
export default function ChaptersPanel({
  ...
  const canSelect = !!onBulkComplete && !!onBulkDelete && chapters.length > 1;
  ...
  return (
    <div data-tour="chapters" className="pb-3">
      <div className={styles.panelHead}>
        {compact ? (
          ...
          <span className={styles.panelTitle}>
            Chapters
            <span className={styles.panelCount}>{chapters.length}</span>
          </span>
        )}
      ...
      <div className="space-y-2 px-2">
        {chapters.length === 0 ? (
          ...
          <>
            {selection.active && onBulkComplete && onBulkDelete ? (
              <ChapterSelectBar
                selection={selection}
              ...
              <p className="text-[11px] text-[var(--clay-muted)] px-2 pb-2">
                Drag to reorder. Tap the ring to mark a chapter complete.
              </p>
            )}
            {chapters.map((ch, i) => {
              ...
              return (
                <ChapterRow
                  ...
                  checked={selection.selected.has(ch.id)}
                  dropTarget={dragOverIndex === i}
                  onOpen={() => handleSelectChapter(i)}
                  onToggleSelect={() => selection.toggle(ch.id)}
                  onToggleComplete={
                    handleToggleChapterComplete
                  ...
                  onTouchStart={(e) =>
                    !selection.active && handleTouchStart(i, e)
                  }
                  onTouchMove={(e) =>
                    !selection.active && handleTouchMove(i, e)
                  }
                  onTouchEnd={() => !selection.active && handleTouchEnd()}
                  leading={<HandleDragIcon isSelected={isSelected} />}
                  trailing={
                    <>
                      {handleToggleChapterLock && (
                        <button
                          className={`transition-all p-2 rounded-chip flex-shrink-0 ${
                            isSelected
                              ? "text-white/70 dark:text-[var(--clay-muted)] hover:bg-white/15 dark:hover:bg-black/10"
                              : ch.locked
                        ...
                        (pendingDeleteIndex === i ? (
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                (confirmChapterDelete ?? handleRemoveChapter)(
                                  i,
                                );
                                setPendingDeleteIndex(null);
                              }}
                            ...
                            <button
                              ...
                              className="p-2 rounded-chip text-[var(--clay-muted)] hover:bg-gray-200 dark:hover:bg-[var(--rule)] transition-colors flex-shrink-0"
                              aria-label="Cancel"
                              title="Cancel (Esc)"
                            >
                              <svg
                          ...
                          <button
                            className={`transition-colors p-2 rounded-chip flex-shrink-0 ${
                              isSelected
                                ? "text-[var(--clay-muted)] hover:text-red-400 hover:bg-[var(--rule)]"
                                : "text-[var(--clay-muted)] hover:text-red-400 hover:bg-[var(--rule)]"
                            }`}
                            onClick={(e) => {
                              e.stopPropagation();
