...
// AI-generated, user-editable book profile.
// Characters, locations, style, key facts, and writing rules — all
// auto-extracted from the manuscript and stored per book. The AI refreshes
// this in the background after each autosave if the manuscript changed.
...
import type { ProfileCharacter, ProfileLocation, BookProfile } from "../../../types";
...
type EditingChar = { id: string; name: string; role: string; description: string } | null;
...
export default function ProfileTab({ bookId, userId }: ProfileTabProps) {
  ...
  const generate = useCallback(async (force = false) => {
    ...
    setGenerating(true);
    setGenError(null);
    const result = await ensureBookProfile({ userId, book: loadBookById(userId, bookId)!, force });
    setGenerating(false);
    if (!result.ok) {
  ...
  // Auto-generate on first mount if profile is stale and brief exists.
  useEffect(() => {
  ...
  if (generating && !profile) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3">
        <svg className="w-5 h-5 animate-spin text-gray-400 dark:text-[#737373]" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
        </svg>
        <p className="text-xs text-gray-400 dark:text-[#737373]">Analysing manuscript…</p>
      </div>
    );
  }
  ...
  const saveCharAdd = () => {
    ...
    addProfileCharacter(userId, bookId, { name: newChar.name.trim(), role: newChar.role, description: newChar.description.trim() });
  ...
  const saveLocAdd = () => {
    ...
    // addProfileLocation not yet in bookmindMemory — inline here
    ...
    setProfile(userId, bookId, {
      ...p,
      locations: [...p.locations, { id: `loc-${Date.now()}`, name: newLoc.name.trim(), description: newLoc.description.trim(), source: 'user' }],
    });
  ...
  const saveStyle = () => {
    ...
    const p = getProfile(b);
    if (!p) return;
    setProfile(userId, bookId, { ...p, style: { pov: styleForm.pov, tense: styleForm.tense, tone: styleForm.tone } });
    ...
    <div className="flex flex-col h-full overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-300 dark:scrollbar-thumb-[#3a3a3a]">
      ...
      {profile && !fresh && !generating && (
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-2 bg-amber-50 dark:bg-[#2a2000] border-b border-amber-200 dark:border-[#4a3a00]">
          <span className="text-2xs text-amber-700 dark:text-amber-400">Manuscript changed — profile may be out of date</span>
          <button onClick={() => generate()} className="text-2xs font-semibold text-amber-700 dark:text-amber-400 hover:underline">Refresh</button>
        </div>
      )}
      ...
      {generating && profile && (
        <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-[#232323] border-b border-gray-100 dark:border-[#2a2a2a]">
          <svg className="w-3 h-3 animate-spin text-gray-400 dark:text-[#737373]" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
          </svg>
          <span className="text-2xs text-gray-400 dark:text-[#737373]">Updating profile…</span>
        </div>
      )}
      ...
      {genError && (
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-2 bg-red-50 dark:bg-[#2a0a0a] border-b border-red-200 dark:border-[#4a1a1a]">
          <span className="text-2xs text-red-600 dark:text-red-400">{genError}</span>
          <button onClick={() => generate(true)} className="text-2xs font-semibold text-red-600 dark:text-red-400 hover:underline">Retry</button>
        </div>
      )}

      {/* Persistent explanation — always visible */}
      <div className="flex-shrink-0 px-4 pt-3 pb-1">
        <p className="text-2xs text-gray-400 dark:text-[#737373] leading-relaxed">
          Everything here is sent to Book Mind with every message — so it always knows your characters, style, and rules without you having to repeat yourself.
        </p>
      </div>
      ...
      <div className="px-4 py-4 space-y-6">
        ...
        {profile?.style && (
          <section>
            <div className="flex items-center justify-between mb-2">
              <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3]">Style</p>
              {!editingStyle && (
                <button
                  onClick={() => { setStyleForm({ pov: profile.style.pov, tense: profile.style.tense, tone: profile.style.tone }); setEditingStyle(true); }}
            ...
            {editingStyle ? (
              ...
              <div className="space-y-1">
                {profile.style.pov   && <p className="text-xs text-gray-700 dark:text-[#d4d4d4]"><span className="text-gray-400 dark:text-[#737373]">POV</span> · {profile.style.pov}</p>}
                {profile.style.tense && <p className="text-xs text-gray-700 dark:text-[#d4d4d4]"><span className="text-gray-400 dark:text-[#737373]">Tense</span> · {profile.style.tense}</p>}
                {profile.style.tone  && <p className="text-xs text-gray-700 dark:text-[#d4d4d4]"><span className="text-gray-400 dark:text-[#737373]">Tone</span> · {profile.style.tone}</p>}
              </div>
            )}
          </section>
        )}
        ...
        {/* ── Characters ── */}
        <section>
          <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3] mb-2">Characters</p>
                ...
                {editingChar?.id === char.id ? (
                  ...
                  <div className="group flex items-start gap-2">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
                      ...
                      <button
                        onClick={() => { removeProfileCharacter(userId, bookId, char.id); bump(); }}
        ...
        {((profile?.locations ?? []).length > 0 || addingLoc) && (
          <section>
            <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3] mb-2">Locations</p>
            <div className="space-y-2">
                  ...
                  {editingLoc?.id === loc.id ? (
                    ...
                    <div className="group flex items-start gap-2">
                      ...
                      <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
                        ...
                        <button onClick={() => { removeProfileLocation(userId, bookId, loc.id); bump(); }} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors">
              ...
              {addingLoc ? (
                ...
                <button onClick={() => setAddingLoc(true)} className="text-2xs text-gray-400 dark:text-[#737373] hover:text-gray-700 dark:hover:text-white transition-colors mt-1">+ Add location</button>
              )}
            </div>
          </section>
        )}
        ...
        {/* ── Key facts ── */}
        <section>
          <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3] mb-2">Key facts</p>
        ...
        {/* ── Writing rules ── */}
        <section>
          <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3] mb-1">Writing rules</p>
          <p className="text-2xs text-gray-400 dark:text-[#737373] mb-2">Sent with every Book Mind message. E.g. "no em dashes" or "British spelling".</p>
          <div className="space-y-1 mb-2">
        ...
        {decisions.length > 0 && (
          <section>
            <p className="text-2xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#a3a3a3] mb-1">Remembered</p>
            <p className="text-2xs text-gray-400 dark:text-[#737373] mb-2">Added via "Remember this" in chat.</p>
            <div className="space-y-1">
              {decisions.slice(-5).reverse().map((d, i) => (
                <p key={i} className="text-xs text-gray-600 dark:text-[#a3a3a3] leading-relaxed">{d.note}</p>
              ))}
            </div>
          </section>
        )}

        {/* Refresh button at the bottom when profile exists */}
        {profile && !generating && (
