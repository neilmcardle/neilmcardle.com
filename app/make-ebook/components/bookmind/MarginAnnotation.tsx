...
Do NOT flag grammar, spelling, or punctuation. Return ONLY the JSON array. If nothing notable, return [].`,
  ...
  const TYPE_COLORS: Record<Annotation["type"], string> = {
    craft: "text-[#008ff0] bg-[#008ff0]/10",
  ...
  return (
    <div className="absolute top-0 right-0 w-48 pt-16 pr-2 space-y-2 pointer-events-auto">
      ...
      {visible.map((annotation) => (
        ...
        >
          <span
            className={`text-[9px] font-semibold uppercase tracking-wide px-1 py-1 rounded flex-shrink-0 mt-1 ${TYPE_COLORS[annotation.type] || TYPE_COLORS.craft}`}
          >
            {annotation.type}
          </span>
          ...
          <button
            ...
            className="flex-shrink-0 me-reveal text-gray-400 hover:text-gray-700 dark:hover:text-white transition-colors mt-1"
            title="Dismiss"
          >
            <svg
              className="w-3 h-3"
