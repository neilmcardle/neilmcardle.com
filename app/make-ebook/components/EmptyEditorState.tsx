...
function PaperPanel({
  ...
  const words = pasteValue.trim()
    ? pasteValue.trim().split(/\s+/).filter(Boolean).length
    : 0;
    ...
    <div className="relative flex flex-col bg-[#252525] border border-[#3a3a3a] rounded-[20px] overflow-hidden">
      <div className="flex-1 flex flex-col px-7 sm:px-10 py-9 sm:py-10">
        ...
        <p className="mt-8 text-sm italic text-white/35 leading-relaxed">
          ...
          <span className="not-italic text-white/25">— {quote.author}</span>
        </p>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 min-w-0">
        <span
          className="block text-[15px] text-white"
          style={{ fontFamily: "Georgia, serif" }}
        >
          {label}
        </span>
        <span className="block text-[12px] text-white/40 mt-1">
          {description}
        </span>
      </span>
      <svg
