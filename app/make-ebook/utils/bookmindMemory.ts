...
export function isBriefFresh(book: BookRecord | undefined): boolean {
  ...
  return (
    book.bookmindMemory.brief.manuscriptHash === manuscriptHash(book.chapters)
  );
}
...
export function removeRule(userId: string, bookId: string, rule: string): void {
  ...
  patchMemory(userId, bookId, {
    rules: memory.rules.filter((r) => r !== rule),
  });
}
...
export function removeProfileCharacter(
  userId: string,
  bookId: string,
  charId: string,
): void {
...
export function addProfileCharacter(
  userId: string,
  bookId: string,
  char: Omit<ProfileCharacter, "id" | "source">,
): void {
  const book = loadBookById(userId, bookId);
  if (!book) return;
  const profile = getProfile(book);
  const newChar: ProfileCharacter = {
    ...char,
    id: `char-${Date.now()}`,
    source: "user",
  };
  if (profile) {
    setProfile(userId, bookId, {
      ...profile,
      characters: [...profile.characters, newChar],
    });
  }
}

export function removeProfileLocation(
  userId: string,
  bookId: string,
  locId: string,
): void {
...
export function addProfileRule(
  ...
  setProfile(userId, bookId, {
    ...profile,
    writingRules: [...profile.writingRules, rule],
  });
}
...
export function addProfileFact(
  ...
  setProfile(userId, bookId, {
    ...profile,
    keyFacts: [...profile.keyFacts, fact],
  });
}
...
export function formatMemoryForPrompt(memory: BookMindMemory): string {
  ...
  if (memory.rules.length > 0) {
    parts.push(
      "Notes the author asked you to remember (always follow these):\n" +
