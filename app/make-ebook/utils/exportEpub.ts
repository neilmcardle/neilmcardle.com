...
function normalizeHtmlForEpub(html: string): string {
  try {
    ...
    normalized = normalized
      ...
      .replace(
        /\s*data-(?!back-to-ref|endnote)[^=]*\s*=\s*["'][^"']*["']/gi,
        "",
      );
...
function replaceImgSrcs(
  html: string,
  images: { tag: string; ext: string; data: string; filename: string }[],
) {
  let newHtml = html;
  images.forEach((img) => {
...
export async function exportEpub({
  ...
  const safePublisher =
    publisher && publisher.trim().length > 0 ? publisher.trim() : "N/A";
  ...
  const uniqueImages = Array.from(
    new Map(allImages.map((img) => [img.filename, img])).values(),
  );
  ...
  sortedChapters.forEach((ch, idx) => {
    ...
    const chapterTitle = getChapterTitle(
      ch,
      ch.type === "content" ? contentChapterNumber : 0,
    );
    if (ch.type === "content") {
      ...
      if (ch.title?.toLowerCase() === "endnotes") {
        chapterHtml = chapterHtml.replace(
          ...
          (match, quote, refNumber) => {
            const ref = endnoteReferences.find(
              (r: {
                id: string;
                number: number;
                chapterId: string;
                endnoteId: string;
              }) => r.number === parseInt(refNumber),
            );
        ...
        chapterHtml = chapterHtml.replace(
          ...
          (match, quote, endnoteNumber) => {
            const endnotesChapter = sortedChapters.find(
              (c) => c.title?.toLowerCase() === "endnotes",
            );
            if (endnotesChapter) {
              const endnotesFilename = chapterIdToFilename.get(
                endnotesChapter.id,
              );
              if (endnotesFilename) {
  ...
  const extraXhtmlManifestItems = `<item id="toc" href="${tocPageFilename}" media-type="application/xhtml+xml"/>
