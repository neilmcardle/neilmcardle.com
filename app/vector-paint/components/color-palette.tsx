...
export default function ColorPalette({
  ...
  const [huePosition, setHuePosition] = useState<{ x: number; y: number }>({
    x: 0,
    y: 0,
  });
  ...
  const handleInnerSquareClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const { s, l } = getSaturationLightnessFromPosition(
      x,
      y,
      rect.width,
      rect.height,
